...
const SIGUNGU: Record<string, string[]> = {
  ...
  // 경기도: 구가 있는 시는 법원경매 사이트와 동일하게 "시명 구명" 형태로 등록
  '경기도': [
    '가평군',
  ...
  '충청남도': ['계룡시','공주시','금산군','논산시','당진시','보령시','부여군','서산시','서천군','아산시','예산군','청양군','태안군','홍성군'],
...
const DEFAULT: CrawlerConfig = {
  ...
  max_pages: 1, notify_email: '', min_items_to_notify: 1,
};
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <span className="text-sm shrink-0 w-24 text-muted-foreground">{label}</span>
...
export function CrawlerSettingsSheet() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success('크롤링 설정 저장 완료. 다음 실행부터 적용됩니다.');
      setOpen(false);
    } catch (e) {
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetTrigger
        render={<Button variant="ghost" size="sm" className="h-7 w-7 p-0" title="크롤링 설정" />}
      >
      ...
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        ...
        <div className="space-y-3 pb-24">
          ...
          {/* ── 지역 조건 ── */}
          <Section title="지역 조건" desc="법원 기준 또는 소재지 기준으로 검색합니다.">
            <Row label="검색 기준">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">
                    ...
                    <span className="text-xs ml-1">
                      {config.courts.length > 0 ? `(${config.courts.length}개 선택됨)` : '(미선택 시 전체)'}
                    </span>
                  </span>
          ...
          {/* ── 수집 범위 ── */}
          <Section title="수집 범위" desc="페이지당 10건. 최대 5페이지(50건)까지 설정 가능합니다.">
            <Row label="최대 페이지">
        ...
        {/* 저장 버튼 */}
        <div className="absolute bottom-0 left-0 right-0 px-6 py-4 bg-background/95 backdrop-blur border-t">
